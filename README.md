...
In light of these observations, the results also suggest a more extreme prediction setting that naturally arises in the highest-overlap regime, where negatives differ from positives by only a single node. In this setting, the task becomes **missing-node prediction** in nearly complete interactions: given an interaction with one node removed, the goal is to identify the missing node that completes the original group. This can be viewed as a direct generalization of link prediction from pairwise interactions to higher-order interactions. In this regime, particularly in datasets with substantial higher-order structure, THGN is substantially better suited than TCEN, since the decision depends on the global compatibility of the interaction as a whole rather than on pairwise compatibilities that remain largely unchanged under clique expansion.
