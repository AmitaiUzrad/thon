...
"""
TGN Self-Supervised Training Script for Temporal Link Prediction
================================================================

Purpose: Training script for Temporal Graph Network (TGN) that performs temporal link prediction.
This script orchestrates the complete training pipeline: data loading, batch processing, memory
management, validation, testing, and model checkpointing.

Task: Temporal Link Prediction
------------------------------
Given a temporal graph with edges (u, v, t), predict whether an edge exists at a specific
timestamp. The model learns to distinguish positive edges (that exist) from negative edges
(randomly sampled non-edges) at the same timestamp.

"Self-supervised" meaning: Learns from graph structure itself (predicting edges) rather than
external labels, but still uses supervised learning (positive vs negative edges).

Complete Training Flow with Running Example:
--------------------------------------------
Dataset: 10,000 edges, split: train=7,000, val=1,500, test=1,500
...
Step 2: Training Loop (35 batches: 7,000 edges / 200)
  Batch 0: Edges [(50, 60, t=100), (70, 80, t=150), ..., (90, 100, t=200)] (200 edges)
    - Sample negatives: [23, 45, ..., 67] (200 random non-edges)
    - Compute probabilities:
      * pos_prob = tgn.compute_edge_probabilities(sources, destinations, negatives, ...)
    ...
    - Update memory: Memory now reflects interactions from batch 0
      * Memory[50] updated with message from interaction (50, 60, t=100)
      * Memory[60] updated with message from interaction (50, 60, t=100)
      * Similar for all nodes in batch
  
  Batch 1: Edges [(110, 120, t=250), ..., (130, 140, t=300)] (200 edges)
    - Memory state: Includes updates from batch 0
    - Query time for each edge: t=250, t=300 (uses memory before these times)
...
Step 3: Validation
  - Switch to full graph (includes val/test edges for neighbor finding)
  - Backup training memory
  - Validate on regular nodes (val_data: 1,500 edges)
...
Key Components:
---------------
- Data Loading: Splits temporal graph into train/val/test chronologically
- Neighbor Finders: train_ngh_finder (training edges only) vs full_ngh_finder (all edges)
- Negative Samplers: Randomly sample non-edges for each positive edge (seeded for reproducibility)
...
Important Points:
-----------------
- Memory resets at start of each epoch (line 187): Ensures fair comparison across epochs
- Batches processed chronologically: Earlier batches update memory used by later batches
- Validation uses full graph: Can see val/test edges as neighbors (but not for prediction)
- Memory backup/restore: Allows testing different scenarios (regular vs new nodes) with same memory state
- Within-batch temporal consistency: All edges in batch use same memory state (from before batch)
- Negative sampling: Each positive edge gets one negative edge at same timestamp
- Loss: Binary cross-entropy on edge probabilities (positive=1, negative=0)
...
# For debugging provenance: store the actual train negatives sampled on-the-fly (epoch 0).
train_runtime_negative_destinations_epoch0 = []
  ...
  for epoch in range(NUM_EPOCH):
    ...
    for k in range(0, num_batch, args.backprop_every):
      ...
      for j in range(args.backprop_every):
        ...
        if epoch == 0:
          train_runtime_negative_destinations_epoch0.extend([int(x) for x in negatives_batch])
        ...
        if epoch == 0 and getattr(train_data, "rows_df", None) is not None:
          ...
          batch_rows["runtime_train_neg_u"] = batch_rows["u"].values
          batch_rows["runtime_train_neg_i"] = np.array(negatives_batch, dtype=int)
          # match dtypes to u/i
          try:
            if pd.api.types.is_integer_dtype(batch_rows["u"].dtype):
              batch_rows["runtime_train_neg_u"] = pd.to_numeric(batch_rows["runtime_train_neg_u"], errors="coerce").astype("Int64")
            else:
              batch_rows["runtime_train_neg_u"] = batch_rows["runtime_train_neg_u"].astype(str)
            if pd.api.types.is_integer_dtype(batch_rows["i"].dtype):
              batch_rows["runtime_train_neg_i"] = pd.to_numeric(batch_rows["runtime_train_neg_i"], errors="coerce").astype("Int64")
            else:
              batch_rows["runtime_train_neg_i"] = batch_rows["runtime_train_neg_i"].astype(str)
          except Exception:
            pass
